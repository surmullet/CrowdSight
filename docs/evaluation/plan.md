...
| Crowd held-out evaluation | Video manifest, labels/predictions formats, video inference runner, box scorer, and independence/permission gates are implemented. A selection-to-manifest preparer, hash-checking image-sequence inference runner, and DroneCrowd count-only scorer now cover the frozen candidate path. No verified held-out result exists. The locked 30-frame Mixkit sample has pinned predictions but blank labels, and visual review shows it is useful only as a negative-heavy false-positive diagnostic. A [76-sequence DroneCrowd selection record](manifests/dronecrowd-remaining-train-sequences-candidate-v1.json) freezes 2,280 head-point-labeled frames beyond the six sequences previously used with `best.pt`. The local package has no explicit license, the official 2020 terms remain unclear, and full checkpoint lineage independence is unverified. See the [DroneCrowd candidate review](dronecrowd-candidate-evaluation.md). | Obtain source-specific permission for noncommercial model evaluation and annotation transformation, generate the private manifest from the locked selection, produce complete reviewed labels, and complete the full checkpoint/sequence lineage review. Only then run image-sequence inference and the count scorer; authenticate all evidence before any held-out claim. |
