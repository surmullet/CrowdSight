...
| Crowd held-out evaluation | Versioned manifest, manual labels/predictions format, inference runner, scorer, and independence/permission gates are implemented. No verified held-out result exists. The locked 30-frame Mixkit sample has pinned predictions but blank labels, and visual review shows it is useful only as a negative-heavy false-positive diagnostic. A stronger auxiliary candidate is frozen before new inference in a [76-sequence DroneCrowd selection record](manifests/dronecrowd-remaining-train-sequences-candidate-v1.json), covering 2,280 head-point-labeled frames beyond the six sequences previously used with `best.pt`. The local package has no explicit license, labels are points rather than boxes, and full checkpoint lineage independence is unverified. See the [DroneCrowd candidate review](dronecrowd-candidate-evaluation.md). | Establish source/data-use permission and full model-source independence; implement the point-annotation evaluation path or obtain a permitted box-labeled independent video; then generate predictions for the frozen sequence selection, manually review/adjudicate annotations as required, and score it. Do not claim held-out performance until all evidence references are authenticated. |
