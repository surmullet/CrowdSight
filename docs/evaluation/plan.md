...
| Crowd inference adapter | Versioned profile and hash-verifying detector/tracker adapters are present; inference output records profile/checkpoint hashes and runtime metadata. | Integrator pins a compatible environment for the target platform and records exact runtime/device. Current Windows/CUDA requirements are documented as direct pins, not a complete transitive lock. |
| Crowd held-out evaluation | Versioned manifest, manual labels/predictions format, inference runner, scorer, and independence/permission gates are implemented. No approved held-out result exists: Plaza overlaps training; `best.pt` was previously measured on the six-sequence DroneCrowd comparison; the DroneCrowd challenge annotations are absent; and `heat_map/data/videos/test.mp4` has no reviewed labels. A separately selected, nearly fixed overhead Mixkit intersection video is available in `../uav-crowd-monitoring/artifacts/independent-intersection/` with SHA-256 `e9084a41628e578d45f356e1a9ae1bfb6ad182f3786d09ad4f965aab259fe6a1`; it was not used in E01 fine-tuning, but source independence from upstream pretraining has not been reviewed. A 30-frame uniform sample is locked before CrowdSight inference in the [candidate manifest](manifests/mixkit-busy-intersection-test-v1.manifest.json); a prediction-free [offline review page](../../tools/crowd-frame-review.html) is ready to collect manual labels. Permission remains pending and labels are blank. The clip supports sparse overhead pedestrian diagnostics, not dense-crowd or Vietnam site evidence. | Named data owner completes the license/permission record; reviewer verifies source independence against a complete model-training inventory; annotator completes and adjudicates all 30 frames. Then run the pinned inference and scorer. Results remain exploratory until the evidence references are authenticated. See [candidate video sources](candidate-video-sources.md). |
| Uncertainty and geographic density | Quality states, raw-score semantics, and fail-closed density validity checks are defined. | Site supplies surveyed calibration and area evidence, independent residual review, and policy approval before metric density can be enabled. |
