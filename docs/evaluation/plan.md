...
| Crowd held-out evaluation | Versioned manifest, manual labels/predictions format, inference runner, scorer, and independence/permission gates are implemented. No fresh, approved test set was found locally: Plaza overlaps training; `best.pt` was previously measured on the six-sequence DroneCrowd comparison; the DroneCrowd challenge annotations are absent; and `heat_map/data/videos/test.mp4` has no reviewed labels. Details are in the [model inventory](../models/crowd-model-inventory.md). | Dataset owner supplies an approved independent source partition, source and permission evidence, locked selection, complete training inventory, independent review, and complete manual labels. Then run inference and scoring; reviewer validates evidence before any held-out claim. |
