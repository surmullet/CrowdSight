...
| Crowd held-out evaluation | MOT20-05 exploratory evaluation v4 completed on 134 sampled frames with publisher ground truth: 8,240 TP, 4,054 FP, 17,880 FN, precision 0.6702, recall 0.3155, count MAE 103.18, RMSE 105.04, bias -103.18. Raw-score calibration is conditional on emitted detections and does not authorize probability claims. See [MOT20 diagnostic report](../models/crowd-best-local-mot20-diagnostic.md). | No verified held-out result exists. Treat MOT20 as exploratory. A named reviewer must authenticate data terms and review complete checkpoint lineage/sequence independence before any held-out claim. |
...
**MOT20 acquisition/evaluation note (2026-09-26):** following product-lead authorization, official video and label archives were downloaded to private storage, CRC-checked, and used for exploratory MOT20-05 evaluation. The v4 manifest records license terms as unverified, and the evaluator marks evidence review required. Exact hashes and private location are in the diagnostic report. Current official archive page omits license terms; historical terms are unauthenticated. Human review and complete checkpoint lineage review remain required. No assets or results are in Git; this is not held-out evidence.
...
- The current raw-score calibration scorer writes equal-width reliability-bin values (`n`, `mean_score`, `observed_accuracy`) and can render an optional, dependency-free SVG reliability diagram with `--reliability-svg`. The plot shows non-empty overall bins, mean raw score against observed matched-box accuracy, with sample counts in point tooltips; outputs remain descriptive. If confidence is later presented as probability, fit a versioned calibrator on a separate calibration partition, label predictions using the declared one-to-one IoU rule, then report a reviewed reliability diagram, Brier score, negative log-likelihood, declared-bin ECE, sample counts, and operating-condition strata on the locked independent test partition. Store the calibrator ID/hash, calibration-data manifest hash, matching rule, SVG and test report. Site-owner approval is required before a calibrated-probability claim; v1 application contracts currently permit only `RAW_MODEL_SCORE`.
