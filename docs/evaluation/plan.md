...
- The current raw-score calibration scorer writes equal-width reliability-bin values (`n`, `mean_score`, `observed_accuracy`) that can be plotted; it does not render a diagram. If confidence is later presented as probability, fit a versioned calibrator on a separate calibration partition, label predictions using the declared one-to-one IoU rule, then report an actual reliability diagram, Brier score, negative log-likelihood, declared-bin ECE, sample counts, and operating-condition strata on the locked independent test partition. Store the calibrator ID/hash, calibration-data manifest hash, matching rule, and test report. Site-owner approval is required before a calibrated-probability claim; v1 application contracts currently permit only `RAW_MODEL_SCORE`.
