...
| Crowd held-out evaluation | Versioned manifest, manual labels/predictions format, inference runner, scorer, and independence/permission gates are implemented. No verified held-out result exists. The locked 30-frame Mixkit sample has pinned predictions but blank labels, and visual review shows it is useful only as a negative-heavy false-positive diagnostic. A stronger auxiliary candidate exists in 76 DroneCrowd sequences (2,280 annotated frames) beyond the six sequences previously used with `best.pt`; however, the local package has no explicit license, labels are head points rather than boxes, and full checkpoint lineage independence is unverified. See the [DroneCrowd candidate review](dronecrowd-candidate-evaluation.md). | Establish source/data-use permission and full model-source independence; implement the point-annotation evaluation path or obtain a permitted box-labeled independent video; then lock a sequence-level sample, generate predictions, manually review/adjudicate labels as required, and score it. Do not claim held-out performance until all evidence references are authenticated. |
