...
Contract v1 serializes `confidence_semantics: RAW_MODEL_SCORE` only. The inference archive preserves each raw detection score for evaluation, and `scripts/evaluate_confidence_calibration.py` measures Brier score, negative log-likelihood, and equal-width ECE against reviewed boxes by IoU matching. Its JSON report includes per-bin sample count, mean raw score, and observed accuracy; the optional `--reliability-svg` argument writes a diagram of the non-empty overall bins with per-point sample counts. These diagnostics are conditional on emitted detections, do not fit a calibrator, and do not turn the raw score into a probability. A calibrated-probability output requires a reviewed contract revision, a versioned calibrator trained on a separate calibration partition, and independent test evidence (reviewed reliability diagram, Brier score, negative log-likelihood, declared-bin ECE, sample counts, and condition-stratified results) plus site-owner approval. Until those gates pass, consumers must display the value as a raw score or omit it.
