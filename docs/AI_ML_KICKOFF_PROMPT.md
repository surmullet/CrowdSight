...
5. **Waiting-time extension:** treat customer waiting-time estimation as separate downstream queue analytics, not a parking occupancy model output. Define queue meaning, event/throughput inputs, owners, uncertainty, and evaluation data before proposing an estimator or adding a schema.
6. **Application contracts:** coordinate proposed crowd and parking schemas with backend/frontend owners. Preserve the proposed status until field, time, freshness, quality, confidence, box/heat-map transport, storage, and versioning decisions have named reviewers and dates recorded. Do not silently freeze or extend shared schemas.
...
- An exploratory MOT20-05 v4 evaluation completed on 134 locked frames: precision 0.6702, recall 0.3155, count MAE 103.18, count bias -103.18. It is publisher-ground-truth external-domain evidence, not verified held-out or Vietnam/site validation; source terms and checkpoint independence need review. See the MOT20 diagnostic report at models/crowd-best-local-mot20-diagnostic.md.
...
- The locked Mixkit video and its 30-frame manifest/predictions are outside Git. The labels template is still blank. Original-resolution review of all 30 locked frames found small but visible pedestrians in several sidewalk/crosswalk frames; the private SAM 3.1 run produced zero proposals, likely including misses. There are no reviewed ground-truth labels, so this clip cannot support accuracy metrics or held-out claims.
- The sibling workspace has 82 annotated DroneCrowd training sequences. Six were included in an earlier `best.pt` comparison; the other 76 (2,280 frames) are frozen in a pre-inference candidate selection record. The package has no explicit local license, point labels require a separate scorer, and full checkpoint-source independence remains unknown. See [`dronecrowd-candidate-evaluation.md`](evaluation/dronecrowd-candidate-evaluation.md); do not use it until its gates pass.
...
- A bounded SAM 3.1 annotation-assistance run completed on Kaggle T4 for the 30 locked Mixkit frames. It produced zero person proposals, not reviewed labels or accuracy evidence. The archive, run log, and provenance are in the private artifact bundle outside Git; see [`sam3-kaggle-annotation.md`](evaluation/sam3-kaggle-annotation.md). Treat all SAM outputs as pseudo-labels and require frame-by-frame human review.
